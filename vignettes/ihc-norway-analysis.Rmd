...
## Appendix: Notes on comparisons to Ugandans

1. I initially tried to use the `svyglm` function of the `survey` package to compare
   Norwegians to Ugandans, as this would allow post-stratification (and modelling the
   clustering as a "complex design"). However, I found that the point estimates provided
   were incorrect and the standard errors were all identical and very small. The standard
   errors provided may be the variances between the distinct multiply-imputed data
   sets, but in any case, the point estimates were clearly wrong.
2. As a sanity-check, I compared the results from `svyglm` with `glm` (i.e., a regular
   generalized linear model). The `glm` function provided sensible-looking point estimates
   and standard errors, but does not account for the clustering structure.
3. I then moved to using the `lme4` package, also provides sensible point estimates and
   standard errors, allows the clustering structure to be modelled, but does not allow
   post-stratification weights to be used for the Norwegians. The results presented can
   therefore not be interpreted as a comparison of typical Norwegians to Ugandans, but as
   a comparison of Norwegians who answer surveys to Ugandans (who take part in research
   studies).
4. The specific models fitted using the `lme4` package do not adjust for variables such
   as education. I tried fitting such models, but the software was unable to do so, likely
   because of collinearity (i.e., education of the primary school children can be
   predicted perfectly by the fact they are primary school children).
5. The clustering actually has a more complex structure than modelled (randomized units are
   clustered within country), so ideally we would estimate distinct variances for the
   random intercepts within Uganda and within Norway. However, the software was unable to
   fit this model, perhaps because of collinearity between group and country (i.e.,
   it is possible predict an individual's country given their group membership). A common
   variance is therefore assumed for the random intercepts.

```{r echo=FALSE, message=FALSE, fig.width = 10, fig.height = 2}
### >80% Answered correctly
# understand_concepts_result %>% dplyr::filter(proportion > 0.8) %>% plot(simple = TRUE)
```

```{r echo=FALSE, message=FALSE, fig.width = 7, fig.height = 2.5}
### No better than guessing
# understand_concepts_result %>% dplyr::filter(no_better_than_random) %>% plot(simple = TRUE)
```

```{r echo=FALSE, message=FALSE, fig.width = 7, fig.height = 2.5}
### Worse or no better than Ugandan children
# **NOTE** This result is "hard-coded" and based on preliminary data. Please check that the
# result is correct before using this.
# understand_concepts_result %>% plot(simple = TRUE, concept_subset = c(
#   "A study shows!",
#   "Old is better!",
#   "New is better!",
#   "It worked for me!",
...
```{r echo=FALSE, message=FALSE, fig.width = 10, fig.height = 2.5}
### Better than Ugandan children
...
#   "Dissimilar expectations or behaviours",
#   "Few people or events",
#   "Do the advantages outweigh the disadvantages for you?"
#   ))
```
